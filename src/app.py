...
from core.naver_scraper import (
    get_naver_discussion_by_likes,
    get_foreign_net_buying,
    get_news_vacuum,
...
from ui.main_panel import (
    ...
    render_fundamental_stats,
    render_news_headlines,
)
from ui.sidebar_cards import (
    render_entry_card,
    render_gauge_and_tier,
    render_score_history_sparkline,
    render_score_metrics,
    render_signal_summary,
    render_fomo_panel,
    render_indicator_group,
    render_risk_card,
)
...
is_detail_view = bool(st.session_state.get("dashboard_ready")) or (
    "stock" in st.query_params
)
...
try:
    ...
    volume_series = (
        df["Volume"].squeeze() if "Volume" in df.columns else pd.Series(dtype=float)
    )
    ...
    risk_levels = calculate_risk_levels(
        close_cleaned, high_cleaned, low_cleaned, current_price
    )
    entry = get_entry_signal(obj_indicators, final_scream_score, risk_levels)

    price_keys = [
        ("rsi", "fa-solid fa-wave-square", "RSI (14일)"),
        ("bb", "fa-solid fa-up-down", "볼린저 밴드"),
        ("w52", "fa-solid fa-circle-arrow-down", "52주 신저가"),
        ("drawdown", "fa-solid fa-arrow-trend-down", "고점 대비 낙폭"),
        ("trend", "fa-solid fa-chart-line", "장기추세 (200일선)"),
        ("candle", "fa-solid fa-chart-column", "캔들패턴"),
    ]
    if "ichimoku" in obj_indicators:
        price_keys.append(("ichimoku", "fa-solid fa-cloud", "일목균형표"))
    price_keys.append(("rs", "fa-solid fa-scale-balanced", "시장대비 상대강도"))

    supply_keys = [
        ("volume", "fa-solid fa-bolt", "거래량 폭발"),
        ("pvd", "fa-solid fa-triangle-exclamation", "공포-거래량 괴리"),
        ("foreign", "fa-solid fa-globe", "외국인 동향"),
    ]
    if "obv" in obj_indicators:
        supply_keys.append(("obv", "fa-solid fa-chart-area", "OBV 다이버전스"))

    col_main, col_side = st.columns([6, 4])

    with col_main:
        render_company_header(selected_company, ticker_input, sector)
        chart_mode = render_tab_group(
            ["라인", "캔들"], key="chart_mode_toggle", margin_bottom="-10px"
        )
        ...
        st.markdown(
            f"<hr style='border:none; border-top:1px solid rgba(15,23,42,0.1); margin:16px 0 2px 0;'>",
            unsafe_allow_html=True,
        )

        indicator_mode = render_tab_group(
            ["가격 기반", "수급 기반"], key="indicator_mode_toggle"
        )
    ...
    with col_side:
        ...
        render_score_metrics(
            community_raw, objective_score, weight_reduced=bool(volatility_warning)
        )
        ...
        st.markdown(
            ...
            <div style="display:flex; align-items:center; gap:8px; margin:18px 0 10px 0;">
                <span style="font-size:11px; font-weight:800; color:{THEME['text_sub']}; letter-spacing:0.5px;">상세 데이터</span>
