...
from constants import (
    ...
    SCORE_RSI_HOT,
    SCORE_RSI_COLD,
    SCORE_PANIC_SELL_PVD,
    SCORE_SUPPLY_FEAR_FOREIGN,
    SCORE_HIGH_DRAWDOWN,
    SCORE_FOREIGN_STRONG,
    SCORE_VOL_STRONG,
    SCORE_OBV_STRONG,
    SCORE_COMMUNITY_HIGH,
    SCORE_COMMUNITY_LOW,
    SCORE_ADJ_MIN,
    SCORE_ADJ_MAX,
    SCORE_KOSDAQ_ADJ,
...
def _extract_score_flags(obj_indicators, fomo_score):
    ...
    return {
        ...
        "is_structural_downtrend": obj_indicators.get("trend", {}).get(
...
def _determine_base_score(f):
    ...
    if is_near_high and is_fomo_hot and is_rsi_hot:
        return 15
    elif is_near_high and is_fomo_hot:
        return 22
    elif is_near_high and is_rsi_hot:
        return 28
    elif is_near_high:
        return 35
    elif is_near_low and is_panic_sell and is_rsi_cold:
        return 88
    elif is_near_low and is_panic_sell:
        return 78
    elif is_near_low and is_rsi_cold:
        return 72
    elif is_near_low:
        return 63
    elif is_high_drawdown and is_panic_sell and is_rsi_cold:
        return 75
    elif is_high_drawdown and is_rsi_cold:
        return 65
    elif is_high_drawdown and is_panic_sell:
        return 68
    elif is_high_drawdown and bb_score >= 8:
        return 60
    elif is_high_drawdown:
        return 55
    elif is_panic_sell and is_rsi_cold:
        return 70
    elif is_rsi_cold and bb_score >= 8:
        return 65
    elif is_rsi_hot and is_fomo_hot:
        return 25
    elif is_rsi_hot:
        return 38
    else:
...
def _compute_score_adjustment(f, community_raw):
    ...
    if f["is_supply_fear"]:
        adj += 8
    if f["foreign_score"] >= SCORE_FOREIGN_STRONG:
        adj += 4
    if f["vol_score"] >= SCORE_VOL_STRONG:
        adj += 4
    if f["obv_score"] >= SCORE_OBV_STRONG:
        adj += 3
    if f["is_fomo_hot"]:
        adj -= 7
    if community_raw >= SCORE_COMMUNITY_HIGH:
        adj += 4
    if community_raw <= SCORE_COMMUNITY_LOW:
        adj -= 4
    if f["is_high_drawdown"]:
        adj += 5
...
def calculate_final_score(obj_indicators, community_raw, is_kosdaq, fomo_score):
    ...
    final_scream_score = int(
        max(SCORE_FINAL_MIN, min(SCORE_FINAL_MAX, base + adj + kosdaq_adj))
    )
...
def get_entry_signal(obj_indicators, final_scream_score, risk_levels=None):
    ...
    rr_poor = (
        ...
        and risk_levels["rr_ratio"] < 1.5
        and risk_levels["rr_ratio"] > 0
    )
    rr_too_bad = risk_levels is not None and 0 < risk_levels["rr_ratio"] <= 0.5
    ...
    if rr_too_bad:
        return {
            "level": "🔴 진입 보류",
            "color": "#ef4444",
            "desc": f"손익비 {risk_levels['rr_ratio']} — 목표 대비 손절폭 과다, 타점 부적합",
            "action": "손절/목표 라인 재계산 전까지 진입 금지",
        }
